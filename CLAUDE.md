...
- **Permission mappings live in the backend** — this package defines the `Permission` enum and `canAssign*()` helpers. Role → permission mappings are a deployment concern and live in `flatie-backend/src/shared/constants/permission-mappings.ts`.
