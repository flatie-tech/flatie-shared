...
var apartmentRoleSchema = zod.z.enum([chunkQIDNFYBV_cjs.ApartmentRole.OWNER, chunkQIDNFYBV_cjs.ApartmentRole.TENANT]).describe("`OWNER` for the title-deed holder, `TENANT` for a resident renting from the owner.");
...
var orgRoleSchema = zod.z.enum([chunkQIDNFYBV_cjs.OrgRole.ORG_ADMIN, chunkQIDNFYBV_cjs.OrgRole.SUPERVISOR, chunkQIDNFYBV_cjs.OrgRole.REFERENT, chunkQIDNFYBV_cjs.OrgRole.OPERATIVE]).describe(
...
var createOrganizationSchema = zod.z.object({
  ...
  type: zod.z.enum([chunkQIDNFYBV_cjs.OrgType.MANAGEMENT_FIRM, chunkQIDNFYBV_cjs.OrgType.PLATFORM]).describe(
...
var createBuildingSchema = zod.z.object({
  ...
  role: zod.z.enum([
    chunkQIDNFYBV_cjs.BuildingRole.OWNER_REPRESENTATIVE,
    chunkQIDNFYBV_cjs.BuildingRole.DEPUTY_REPRESENTATIVE,
    chunkQIDNFYBV_cjs.BuildingRole.CO_OWNER
  ]).optional().describe(
...
var updateBuildingSchema = zod.z.object({
  ...
  fundsSource: zod.z.enum([chunkQIDNFYBV_cjs.FundsSource.MANUAL, chunkQIDNFYBV_cjs.FundsSource.CAMT]).optional().describe(
  ...
  pricuvaRefMode: zod.z.enum([chunkQIDNFYBV_cjs.PricuvaRefMode.APARTMENT, chunkQIDNFYBV_cjs.PricuvaRefMode.OWNER]).optional().describe(
...
var buildingQuotaEntrySchema = zod.z.object({
  resourceType: zod.z.enum(
    chunkQIDNFYBV_cjs.QUOTA_RESOURCE_TYPES
  ),
  dailyLimit: zod.z.number().int().min(0).max(1e4).nullable()
});
var buildingQuotaConfigSchema = zod.z.object({
  quotas: zod.z.array(buildingQuotaEntrySchema).max(chunkQIDNFYBV_cjs.QUOTA_RESOURCE_TYPES.length)
});
...
function refineLocation(schema) {
  return schema.superRefine((data, ctx) => {
    if (data.locationType === chunkQIDNFYBV_cjs.FailureLocationType.COMMON_AREA) {
    ...
    if (data.locationType === chunkQIDNFYBV_cjs.FailureLocationType.OWN_UNIT) {
      if (!data.unitType) {
        ctx.addIssue({
          code: "custom",
...
var createFailureReportSchema = refineLocation(
  zod.z.object({
    ...
    priority: zod.z.enum([chunkQIDNFYBV_cjs.Priority.NORMAL, chunkQIDNFYBV_cjs.Priority.URGENT]).optional().describe("`normal` for standard reports, `urgent` to flag immediate attention."),
    locationType: zod.z.enum([chunkQIDNFYBV_cjs.FailureLocationType.COMMON_AREA, chunkQIDNFYBV_cjs.FailureLocationType.OWN_UNIT]).optional().describe(
    ...
    unitType: zod.z.enum([chunkQIDNFYBV_cjs.FailureUnitType.APARTMENT, chunkQIDNFYBV_cjs.FailureUnitType.GARAGE, chunkQIDNFYBV_cjs.FailureUnitType.STORAGE_UNIT]).optional().describe("Kind of unit when `locationType` is `own_unit`. Required in that case."),
...
var updateFailureReportSchema = refineLocation(
  zod.z.object({
    ...
    priority: zod.z.enum([chunkQIDNFYBV_cjs.Priority.NORMAL, chunkQIDNFYBV_cjs.Priority.URGENT]).optional().describe("Revised priority: `normal` or `urgent`."),
    locationType: zod.z.enum([chunkQIDNFYBV_cjs.FailureLocationType.COMMON_AREA, chunkQIDNFYBV_cjs.FailureLocationType.OWN_UNIT]).optional().describe("Revised location classification: `common_area` or `own_unit`."),
    ...
    unitType: zod.z.enum([chunkQIDNFYBV_cjs.FailureUnitType.APARTMENT, chunkQIDNFYBV_cjs.FailureUnitType.GARAGE, chunkQIDNFYBV_cjs.FailureUnitType.STORAGE_UNIT]).optional().describe("Revised unit kind. Required when `locationType` is `own_unit`."),
...
var orgQuotaEntrySchema = zod.z.object({
  resourceType: zod.z.enum(
    chunkQIDNFYBV_cjs.ORG_QUOTA_RESOURCE_TYPES
  ),
  dailyLimit: zod.z.number().int().min(0).max(1e4).nullable()
});
var orgQuotaConfigSchema = zod.z.object({
  quotas: zod.z.array(orgQuotaEntrySchema).max(chunkQIDNFYBV_cjs.ORG_QUOTA_RESOURCE_TYPES.length)
});
...
var createTransactionCategorySchema = zod.z.object({
  ...
  type: zod.z.enum([chunkQIDNFYBV_cjs.TransactionType.INCOME, chunkQIDNFYBV_cjs.TransactionType.EXPENSE]).describe(
...
var getTransactionCategoriesQuerySchema = zod.z.object({
  type: zod.z.enum([chunkQIDNFYBV_cjs.TransactionType.INCOME, chunkQIDNFYBV_cjs.TransactionType.EXPENSE]).optional().describe(
...
var roleTypeSchema = zod.z.enum([
  ...Object.values(chunkQIDNFYBV_cjs.BuildingRole),
  ...Object.values(chunkQIDNFYBV_cjs.OrgRole),
  ...Object.values(chunkQIDNFYBV_cjs.PlatformRole)
]);
...
var buildingStatusSchema = zod.z.enum(Object.values(chunkQIDNFYBV_cjs.BuildingStatus)).describe(
...
({
  [chunkQIDNFYBV_cjs.NotificationType.NOTICE_CREATED]: noticeCreatedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.NOTICE_APPROVED]: noticeApprovedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.NOTICE_REJECTED]: noticeRejectedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.POLL_CREATED]: pollCreatedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.POLL_DEADLINE_24H]: unimplementedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.POLL_DEADLINE_1H]: unimplementedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.POLL_FINALIZED]: pollFinalizedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.EVENT_CREATED]: eventCreatedOrUpdatedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.EVENT_UPDATED]: eventCreatedOrUpdatedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.EVENT_CANCELLED]: eventCancelledDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.EVENT_REMINDER_24H]: unimplementedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.EVENT_REMINDER_1H]: unimplementedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.WASTE_REMINDER_MIXED]: wasteReminderDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.WASTE_REMINDER_BIO]: wasteReminderDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.WASTE_REMINDER_PLASTIC_METAL]: wasteReminderDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.WASTE_REMINDER_PAPER_CARDBOARD]: wasteReminderDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.FAILURE_REPORT_CREATED]: failureReportCreatedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.FAILURE_REPORT_STATUS_CHANGED]: failureReportStatusDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.FAILURE_REPORT_RESOLVED]: failureReportStatusDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.MAINTENANCE_LOG_CREATED]: maintenanceLogCreatedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.PAYMENT_DUE]: unimplementedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.PAYMENT_RECEIVED]: unimplementedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.BUILDING_JOIN_REQUEST_RECEIVED]: buildingJoinRequestReceivedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.BUILDING_JOIN_REQUEST_APPROVED]: buildingJoinRequestDecidedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.BUILDING_JOIN_REQUEST_REJECTED]: buildingJoinRequestDecidedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.BUILDING_MEMBER_JOINED]: buildingMemberJoinedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.BUILDING_ROLE_CHANGED]: buildingRoleChangedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.BUILDING_PENDING_APPROVAL]: buildingPendingApprovalDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.BUILDING_APPROVED]: buildingApprovedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.BUILDING_REJECTED]: buildingRejectedDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.CHAT_MESSAGE]: chatMessageDataSchema,
  [chunkQIDNFYBV_cjs.NotificationType.SYSTEM_ANNOUNCEMENT]: unimplementedDataSchema
});
...
var notificationTypeValues = Object.values(chunkQIDNFYBV_cjs.NotificationType);
...
//# sourceMappingURL=chunk-2ANRV7LD.cjs.map
//# sourceMappingURL=chunk-2ANRV7LD.cjs.map