...
var booleanish = z.union([z.boolean(), z.literal("true"), z.literal("false")]).transform((value) => typeof value === "boolean" ? value : value === "true");
...
var bugReportStatusSchema = z.enum(BUG_REPORT_STATUSES).describe("Triage state of the report.");
...
var createBugReportSchema = z.object({
  title: z.string().trim().min(1).max(BUG_REPORT_LIMITS.TITLE_MAX).describe("One-line summary of the problem."),
  description: z.string().trim().min(1).max(BUG_REPORT_LIMITS.DESCRIPTION_MAX).describe("What happened, what was expected \u2014 free text."),
  url: z.string().trim().max(BUG_REPORT_LIMITS.URL_MAX).describe("The in-app URL the report was filed from, captured automatically."),
  viewport: z.string().trim().max(40).optional().describe('Client viewport at report time, e.g. "1440x900", captured automatically.')
}).meta({ id: "CreateBugReport" });
var updateBugReportSchema = z.object({
  status: bugReportStatusSchema.optional().describe("New triage state; omit to keep."),
  adminNote: z.string().trim().max(BUG_REPORT_LIMITS.NOTE_MAX).optional().nullable().describe("Internal triage note; null clears it, omit keeps it.")
}).meta({ id: "UpdateBugReport" });
var bugReportResponseSchema = z.looseObject({
  ...
  screenshotUrl: z.string().nullable().describe("Presigned URL of the attached screenshot; null when none was attached."),
...
var listBugReportsResponseSchema = z.object({
...
var periodSchema2 = z.string().regex(/^\d{4}-\d{2}$/, "Period must be YYYY-MM.");
var isoDateSchema3 = z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Date must be YYYY-MM-DD.");
var feeModelSchema = z.enum([FeeModel.PER_UNIT, FeeModel.PER_SQM, FeeModel.FLAT]);
var managementInvoiceStatusSchema = z.enum([
  ManagementInvoiceStatus.ISSUED,
  ManagementInvoiceStatus.PAID,
  ManagementInvoiceStatus.CANCELLED
]);
var managementInvoiceSchema = z.object({
  id: uuidSchema,
  orgId: uuidSchema,
  buildingId: uuidSchema,
  buildingName: z.string(),
  ...
  quantity: z.string().describe("Units, m\xB2 or 1 \u2014 two decimals as string."),
...
var managementInvoiceListQuerySchema = z.object({
  status: managementInvoiceStatusSchema.optional(),
  buildingId: uuidSchema.optional(),
  year: z.coerce.number().int().min(2e3).max(2100).optional(),
...
var managementInvoiceSummarySchema = z.object({
...
var previewManagementInvoicesSchema = z.object({
...
var managementInvoicePreviewRowSchema = z.object({
  buildingId: uuidSchema,
  buildingName: z.string(),
...
var previewManagementInvoicesResponseSchema = z.object({
  orgId: uuidSchema,
  period: periodSchema2,
...
var issueManagementInvoicesSchema = z.object({
  period: periodSchema2,
  buildingIds: z.array(uuidSchema).min(1).max(500),
  sendEmail: z.boolean().optional().describe("Also email the PDF to the building\u2019s representatives.")
}).meta({ id: "IssueManagementInvoices" });
var issueManagementInvoicesResponseSchema = z.object({
...
function surfaceCoef() {
  return clearableNumber(z.coerce.number().nonnegative().optional(), void 0).optional();
}
var BUILDING_LIMITS = {
  NAME_MIN: 1,
  NAME_MAX: 100,
...
var orgUplatniceBuildingRowSchema = z.object({
  buildingId: uuidSchema,
  buildingName: z.string(),
...
var orgUplatniceOverviewResponseSchema = z.object({
  orgId: uuidSchema,
...
var orgUplatniceQuerySchema = z.object({
  period: z.string().regex(/^\d{4}-\d{2}$/).optional().describe("YYYY-MM; defaults to the current month."),
  /** Comma-separated building ids for the ZIP variant; omitted = every ready building. */
  buildingIds: z.string().optional()
});
var sendOrgUplatniceSchema = z.object({
  period: z.string().regex(/^\d{4}-\d{2}$/),
...
var orgUplatniceSendResultSchema = z.object({
  buildingId: uuidSchema,
  buildingName: z.string(),
...
var sendOrgUplatniceResponseSchema = z.object({
  period: z.string(),
...
var organizationInvoicingIdentitySchema = z.object({
  ...
  iban: z.string().trim().regex(/^[A-Z]{2}\d{2}[A-Z0-9]{11,30}$/, "IBAN must be uppercase, no spaces").nullable().optional().describe("Account the fee is paid to (payee IBAN on the HUB3 slip)."),
  ...
  courtRegister: z.string().trim().max(200).nullable().optional().describe("Registration court + MBS, printed in the invoice footer."),
  invoiceBusinessUnitCode: z.string().trim().max(20).optional().describe(
    'Oznaka poslovnog prostora in the invoice number ({seq}/{bu}/{device}). Default "1".'
  ),
  invoiceDeviceCode: z.string().trim().max(20).optional().describe('Oznaka naplatnog ure\u0111aja. Default "1".'),
  invoiceDueDays: z.number().int().min(1).max(90).optional().describe("Payment term, days from issue."),
  invoiceFooter: z.string().trim().max(500).nullable().optional(),
  eposlovanjeApiKey: z.string().trim().min(8).max(200).nullable().optional().describe(
    "ePoslovanje.hr API key for sending eRa\u010Dun on the org\u2019s behalf. Write-only; null clears."
  ),
  autoIssueInvoices: z.boolean().optional().describe("Issue fee invoices automatically on the 1st for the previous month.")
});
...
var contractFeeFields = {
  feeModel: z.enum([FeeModel.PER_UNIT, FeeModel.PER_SQM, FeeModel.FLAT]).nullable().optional().describe("How the monthly fee is priced; null = no fee configured."),
  ...
  feeVatRate: z.number().min(0).max(30).nullable().optional().describe("VAT % applied on the fee (25 for VAT payers, 0 otherwise). Defaults from the org."),
  feeNote: z.string().trim().max(300).nullable().optional().describe("Printed on the invoice line.")
};
...
var pricuvaOpeningBalanceRowSchema = z.object({
...
var pricuvaOpeningBalancesResponseSchema = z.object({
  buildingId: uuidSchema,
  rows: z.array(pricuvaOpeningBalanceRowSchema).describe("One row per owner that has a recorded opening balance.")
}).meta({ id: "PricuvaOpeningBalancesResponse" });
var upsertPricuvaOpeningBalancesSchema = z.object({
  balances: z.array(
    z.object({
      ownerId: uuidSchema.describe("Owner record the balance belongs to."),
      amount: z.number().min(-1e6).max(1e6).describe("EUR; positive = debt, negative = credit, 0 = remove the row."),
      note: z.string().trim().max(500).optional().nullable().describe("Provenance note shown alongside the balance.")
    })
  ).min(1).max(500).describe("Balances to upsert; owners not listed are left untouched.")
}).meta({ id: "UpsertPricuvaOpeningBalances" });
var postPricuvaChargesResponseSchema = z.object({
...
var updateBuildingSettingsSchema = z.object({
  ...
  coOwnerConsensusEnabled: z.boolean().optional().describe(
...
var createFailureReportSchema = refineLocation(
  z.object({
    ...
    contractor: z.string().max(FAILURE_REPORT_LIMITS.CONTRACTOR_MAX).optional().describe('Who did the work ("izvo\u0111a\u010D"), free text. Record-keeping only.'),
...
var submitIdCardVerificationSchema = z.object({
  idCardNumber: z.string().min(9, "ID card number must be 9 characters").max(9, "ID card number must be 9 characters").regex(/^\d{9}$/, "ID card number must be exactly 9 digits")
}).meta({ id: "SubmitIdCardVerification" });
var rejectIdCardVerificationSchema = z.object({
  reason: z.string().min(1).max(500)
}).meta({ id: "RejectIdCardVerification" });
var idCardVerificationStatusSchema = z.enum(["pending", "approved", "rejected"]);
...
var voteWithIdCardSchema = votePollSchema.extend({
  idCardNumber: z.string().regex(/^\d{9}$/, "ID card number must be exactly 9 digits").describe(
...
var updatePlatformFeatureRequestSchema = z.object({
  enabled: z.boolean().describe("New platform-wide state."),
  note: z.string().max(500).optional().describe("Optional reason, shown on /platform/features and recorded in the audit log.")
}).strict();
...
var buildingDetailResponseSchema = z.looseObject({
  ...
  pricuvaTrackingFrom: z.string().regex(/^\d{4}-\d{2}$/).nullable().optional().describe(
...
var buildingFundsLedgerRowSchema = z.object({
  ...
  paidSinceStart: z.number().optional().describe(
  ...
  balance: z.number().optional().describe("openingBalance + chargedTotal \u2212 paidSinceStart. Positive = the owner owes.")
...
var buildingFundsLedgerResponseSchema = z.object({
  ...
  rows: z.array(buildingFundsLedgerRowSchema).describe(
...
var buildingSettingsResponseSchema = z.looseObject({
  ...
  emailEnabled: z.boolean().default(false).describe(
  ...
  coOwnerConsensusEnabled: z.boolean().describe(
    "When true, co-owners can start consensus (binding) polls. When false (default), co-owners can only start community (non-binding) polls."
  ),
...
var failureReportResponseSchema = z.looseObject({
  ...
  warrantyClaim: z.boolean().default(false).describe(
  ...
  contractor: z.string().optional().nullable().describe('Who did the work ("izvo\u0111a\u010D"); null when not recorded.'),
  cost: z.string().optional().nullable().describe(
...
var camtImportResponseSchema = z.looseObject({
  ...
  pricuvaTrackingFromSeeded: z.string().regex(/^\d{4}-\d{2}$/).nullable().optional().describe(
...
var orgStatementImportResultSchema = z.object({
  ...
  status: z.enum(["imported", "not_camt_mode"]).describe(
  ...
  skippedCount: z.number().int().nonnegative().describe("Entries already imported earlier (idempotent skip)."),
  errorCount: z.number().int().nonnegative().describe("Entries rejected with errors.")
});
var orgStatementImportResponseSchema = z.object({
  results: z.array(orgStatementImportResultSchema).describe("One row per org building whose IBAN appeared in the file."),
  unmatchedIbans: z.array(
    z.object({
...
var unmatchedPricuvaRefRowSchema = z.object({
  ...
  samplePayer: z.string().nullable().describe("Description of one of the payments \u2014 usually carries the payer name.")
});
var unmatchedPricuvaRefsResponseSchema = z.object({
...
var mapPricuvaRefSchema = z.object({
  refCode: z.string().trim().regex(/^\d{1,22}$/).describe("The unit segment to adopt as the unit\u2019s paymentRefCode."),
  unitId: z.string().uuid().describe("The unit this code belongs to.")
}).meta({ id: "MapPricuvaRef" });
var mapPricuvaRefResponseSchema = z.object({
  unitId: z.string().uuid().describe("Unit that adopted the code."),
  refCode: z.string().describe("The adopted paymentRefCode."),
  linkedCount: z.number().int().nonnegative().describe("Previously-unmatched income rows retroactively linked to the unit.")
...
var platformFeatureKeySchema = z.enum(PLATFORM_FEATURES).describe("Platform feature key (see PlatformFeature in @flatie/shared/enums).");
var platformFeatureFlagSchema = z.looseObject({
  key: platformFeatureKeySchema,
  enabled: z.boolean().describe("Effective platform-wide state. False = parked/off for everyone."),
  note: z.string().nullable().optional().describe("Free-text reason recorded by the admin who last flipped it."),
  updatedAt: z.string().nullable().optional().describe("ISO-8601 timestamp of the last change; null when never changed."),
  updatedByName: z.string().nullable().optional().describe("Display name of the platform admin who last changed it."),
  /** How many buildings set this feature's per-building toggle away from its default. */
  buildingOverrideCount: z.coerce.number().nullable().optional().describe(
  ...
  buildingOverrideDirection: z.enum(["enabled", "disabled"]).nullable().optional().describe(
...
var platformFeatureFlagsResponseSchema = z.looseObject({
  flags: z.array(platformFeatureFlagSchema).describe("Every known platform feature, whether or not it has a stored row yet.")
}).describe("Admin view of every platform feature flag.");
var featureFlagsResponseSchema = z.looseObject({
  flags: z.record(z.string(), z.boolean()).describe("Feature key \u2192 effective platform-wide enabled state.")
}).describe(
...
var pollScopedUnitSchema = z.looseObject({
  ...
  ownerNames: z.array(z.string()).optional().describe("Display names of owners of this unit; absent when not resolved."),
  allOwnersVoted: z.boolean().optional().describe(
