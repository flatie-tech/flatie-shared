...
var passwordSchema = z.string().min(8).max(100);
...
var createBoardCardSchema = z.object({
  ...
  allowComments: z.boolean().optional().describe("Whether members may comment on this card. Defaults to true."),
...
var incomeAmountSchema = z.coerce.number().min(0, "amount must be >= 0").refine((v) => /^-?\d+(\.\d{1,2})?$/.test(v.toString()), {
...
var createEmailThreadRequestSchema = z.object({
  ...
  ccEmails: z.array(z.string().email()).optional().describe("Optional list of Cc addresses for the first message."),
  subject: z.string().min(1).max(200).describe("Subject line; used for both the first message and the thread summary."),
...
var replyEmailThreadRequestSchema = z.object({
  ...
  ccEmails: z.array(z.string().email()).optional().describe("Optional Cc addresses for this reply; do not persist beyond this message.")
}).strict();
...
var entityLinkMetadataSchema = z.looseObject({
  ...
  amount: z.number().nullable().optional().describe("Monetary amount (e.g. expense/maintenance cost); the client formats as currency."),
